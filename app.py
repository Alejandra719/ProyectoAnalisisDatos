...
# ───────────────────────────────
# Lee datos
DATA_CSV = os.path.join(os.path.dirname(__file__), 'SINIESTROS_FINAL.csv')
...
# Guardar lista de features (opcional)
...
@app.route('/')
def home():
    # Renderiza template con lista de features
    return render_template('index.html', features=top9_features)

@app.route('/predict', methods=['POST'])
def predict():
    data = request.get_json()
    # Construye un DataFrame con los 9 valores recibidos
